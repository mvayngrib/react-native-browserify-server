 ...
 * {Number} browserifyPort (default 8082)
...
function Server(options) {
  ...
  this.browserifyPort = options.browserifyPort || 8082;
  this.entry = options.entry || 'index.ios';
  this.hot = (options.hot === true);
  this.browserifyConfig = options.browserifyConfig;
  ...
  this.webpackBaseURL = 'http://' + this.hostname + ':' + this.browserifyPort;
...
Server.prototype = {
  ...
  start: function() {
    ...
    var port = this.port;
    http.createServer(this.handleRequest.bind(this)).listen(this.port, function() {
  ...
  _writeEntryFile: function(entryModules) {
    var source = ENTRY_JS + '\n';
    var entryFile = this.entryFile = path.join(this.entryDir, this.entry + '.js');
  ...
  _startDevServer: function() {
    return new Promise(function(resolve, reject) {
      // var webpackConfig = this.webpackConfig;
      var webpackURL = 'http://' + this.hostname + ':' + this.browserifyPort;
      var hot = this.hot;
      ...
      // this.webpackServer = new DevServer(compiler, {
      //   hot: hot,
      ...
      // this.webpackServer.listen(this.browserifyPort, this.hostname, function() {
      //   console.log('Webpack dev server listening at ', webpackURL);
      // });

      this._writeEntryFile(['React']);
      var flags = this.browserifyConfig ? this.browserifyConfig.slice() : []
      for (var name in reactNativeExternals) {
        flags.push('-x', reactNativeExternals[name])
      }

      flags.push('--ignore-watch', process.cwd() + '/node_modules/react-native')

      this.server = http.createServer(beefy({
        bundler: 'watchify',
        entries: ['./' + this.entry + '.js'],
        cwd: process.cwd(),
        live: true,
        quiet: false,
        bundlerFlags: flags,
      ...
      this.server.listen(this.browserifyPort, this.hostname, function () {
        console.log('Beefy listening at ', webpackURL);
      })
